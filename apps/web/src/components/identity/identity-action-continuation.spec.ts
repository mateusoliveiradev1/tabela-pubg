...
describe("identity action continuation", () => {
  it("promotes a short typed continuation after step-up and consumes it once", () => {
    ...
    storeIdentityActionContinuation(storage, { kind: "link-email" }, now);

    expect(consumeIdentityActionContinuation(storage, now)).toBeNull();
    expect(promoteIdentityActionContinuation(storage, now + 1_000)).toBe(true);
  ...
  it("rejects expiry, unknown actions and malformed identity ids without replay", () => {
    ...
    expect(promoteIdentityActionContinuation(storage, now + 5 * 60_000)).toBe(false);
    ...
    expect(promoteIdentityActionContinuation(storage, now)).toBe(false);
